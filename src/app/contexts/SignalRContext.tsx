...
export function SignalRProvider({ children }: SignalRProviderProps) {
	...
	const createConnection = async (): Promise<signalR.HubConnection | null> => {
		...
		newConnection.on(
			"UserStartedTyping",
			(typingUser: { userId: string; userName: string }) => {
				if (typingUser.userId !== user?.id) {
					setTypingUsers((prev) => [
						...prev.filter((id) => id !== typingUser.userId),
						typingUser.userId,
					]);
				}
			},
		...
		newConnection.on(
			"UserLeft",
			(leftUser: { userId: string; userName: string }) => {
				console.log(`${leftUser.userName} workspace'den ayrıldı`);
			},
		);

		newConnection.on("MessageError", (error: { error: string }) => {
	...
	const disconnect = useCallback(async (): Promise<void> => {
		if (connection) {
			if (currentWorkspaceId) {
				await connection
					.invoke("LeaveConversation", currentWorkspaceId)
					.catch(console.error);
			}
			await connection.stop();
			setConnection(null);

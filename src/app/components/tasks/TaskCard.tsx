...
export default function TaskCard({ task, theme }: Props) {
	return (
		<div
			className={`group relative rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 border backdrop-blur-sm overflow-hidden transform hover:-translate-y-2 ${
				theme === "dark"
		...
		>
			...
			<div
				className={`absolute top-0 left-0 w-full h-1 ${getPriorityColor(task.priority, theme)} shadow-lg`}
			>
				<div
					className={`absolute inset-0 ${getPriorityColor(task.priority, theme)} blur-sm opacity-50`}
				></div>
			</div>

			{/* Floating priority badge */}
			<div
				className={`absolute top-4 right-4 w-3 h-3 rounded-full ${getPriorityColor(task.priority, theme)} shadow-lg ring-2 ${
					theme === "dark" ? "ring-gray-800" : "ring-white"
				}`}
			>
				<div
					className={`absolute inset-0 rounded-full ${getPriorityColor(task.priority, theme)} animate-ping opacity-30`}
				></div>
			</div>
			...
			<div className="p-6 pt-8">
				<div className="space-y-4">
					...
					<div className="space-y-2">
						<h3
							className={`text-xl font-bold leading-tight group-hover:text-blue-600 transition-colors duration-300 ${
					...
					<div className="flex justify-start">
						<span
							className={`inline-flex items-center px-4 py-2 rounded-full text-xs font-semibold border-2 transition-all duration-300 ${getStatusColor(task.status, theme)}`}
						>
							<div
								className={`w-2 h-2 rounded-full mr-2 ${getPriorityColor(task.priority, theme)}`}
							></div>
					...
					<div className="flex items-center justify-between pt-4 border-t border-opacity-20 border-gray-300">
						...
						<div className="flex items-center space-x-2">
							...
							<button
								type="button"
								className={`p-2 rounded-xl transition-all duration-300 opacity-0 group-hover:opacity-100 ${
									theme === "dark"

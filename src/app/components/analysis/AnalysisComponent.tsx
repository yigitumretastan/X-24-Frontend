		...
		>
			<div className="max-w-7xl mx-auto">
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
					...
					>
						...
						<div className="space-y-4">
							{Object.entries(data.taskAnalytics.tasksByStatus).map(
								([status, count]) => {
									const statusLabels = {
										todo: "Yapılacak",
									...
									const percentage = Math.round(
										(count / data.stats.totalTasks) * 100,
									);
									...
									return (
										...
										>
											<div className="flex items-center space-x-3">
												<div
													className={`w-3 h-3 rounded-full bg-${colors[status as keyof typeof colors]}-500`}
												/>
												<span
													className={`text-sm font-medium ${
					...
					>
						...
						<div className="space-y-3">
							{data.timeAnalytics.dailyHours.slice(-7).map((day, _index) => {
								const date = new Date(day.date);
								const dayName = date.toLocaleDateString("tr-TR", {
									weekday: "short",
								});
								const maxHours = Math.max(
									...data.timeAnalytics.dailyHours.map((d) => d.hours),
								);
								...
								return (
									<div key={day.date} className="flex items-center space-x-3">
										...
										<div
											className={`w-12 text-xs font-semibold text-right ${
												theme === "dark" ? "text-gray-300" : "text-gray-700"
											}`}
										>
											{day.hours}h
				...
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
					{/* Project Progress */}
					<div
						className={`lg:col-span-2 backdrop-blur-sm rounded-2xl p-6 shadow-lg border ${
							theme === "dark"
								? "bg-gray-800/80 border-gray-700/50"
								: "bg-white/80 border-white/20"
						}`}
					>
						...
						<div className="space-y-4">
							{data.projectAnalytics.projectProgress.map((project) => (
								<div
									key={project.name}
								...
								>
									<div className="flex items-center justify-between mb-2">
										...
										<span
											className={`text-sm font-bold text-${getStatusColor(project.status)}-600`}
										>
											%{project.progress}
										</span>
									</div>

...
export default function Sidebar() {
	...
	if (!mounted) {
		return (
			<div
				className={`w-full h-full bg-background border-r ${isCollapsed ? "w-[80px]" : "w-[280px]"}`}
			/>
		);
	}
	...
	return (
		<TooltipProvider delay={0}>
			<div
				className={`w-full h-full flex flex-col bg-background border-r transition-all duration-300 relative ${
			...
			>
				...
				<div
					className={`flex flex-col flex-1 ${isCollapsed ? "px-2" : "px-4"} py-6 space-y-6 overflow-y-auto`}
				>
					{/* Menu Label */}
					{!isCollapsed && (
						<div className="px-4">

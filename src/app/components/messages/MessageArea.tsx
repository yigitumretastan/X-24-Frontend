...
import type { Conversation, Message } from "@/data/messages/conversations";
...
export default function MessageArea({
	...
	const groupMessagesByDate = (messages: Message[]) => {
		const groups: {
			[key: string]: { messages: Message[]; sortOrder: number };
		} = {};
		...
		Object.entries(groups)
			...
			.forEach(([key, value]) => {
				...
				sortedGroups[key] = value.messages.sort(
					(a, b) =>
	...
	const messageGroups = selectedConversation
						...
						>
							<div className="relative group">
								<button
									...
									onClick={() =>
										isSelectMode && handleSelectMessage(message.id)
									}
									onKeyDown={(e) => {
									...
									className={`max-w-xs lg:max-w-md px-4 py-3 rounded-2xl relative text-left ${
										message.senderId === "current_user"
								...
								>
									...
									<div className="flex items-center justify-between mt-1">
										...
										<button
											...
											className={`ml-2 p-1 rounded-full opacity-70 hover:opacity-100 transition-opacity cursor-pointer ${
												message.senderId === "current_user"
													? "hover:bg-white/20"
													: theme === "dark"
														? "hover:bg-gray-600"

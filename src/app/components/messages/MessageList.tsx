...
export default function MessageList({ selectedUser, workspaceId }: MessageListProps) {
  ...
  return (
    ...
    >
      ...
      >
        {(selectedUser === "notlarim" ? localMessages : messages).map((message) => {
          const isUserMessage = message.senderId === user?.id;
          const displayTime = message.timestamp || new Date(message.sentAt).toLocaleTimeString('tr-TR', {
            hour: '2-digit',
            minute: '2-digit'
          });
            ...
            >
              <div
                className={`max-w-xs lg:max-w-md px-4 py-2 rounded-lg ${
                  isUserMessage ? "bg-blue-500 text-white" : "bg-gray-700 text-white"
                }`}
              >
                ...
                <p className="text-xs mt-1 text-right opacity-75">{displayTime}</p>
              </div>
            </div>
          );
        })}

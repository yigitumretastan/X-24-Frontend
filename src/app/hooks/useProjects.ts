	...
	const stats: ProjectStats = useMemo(() => {
		return {
			...
			totalMembers: new Set(
				projects.flatMap((p) => p.assignedMembers.map((m) => m.id)),
			).size,
		...
		return projects.filter((project) => {
			...
			if (filters.dateRange.start || filters.dateRange.end) {
				...
				if (
					...
					projectStart < new Date(filters.dateRange.start)
				) {
					return false;
				}
				if (
					filters.dateRange.end &&
					projectStart > new Date(filters.dateRange.end)
				) {
					return false;
				}
			}
	...
	const createProject = useCallback(async (projectData: CreateProjectData) => {
		try {
			...
			const newProject: Project = {
				...
				milestones:
					projectData.initialMilestones?.map((milestone) => ({
						...milestone,

...
import type { FormErrors, ValidationRules } from "@/app/types/auth";
...
export function useFormValidation<T extends Record<string, unknown>>(
	...
	const getPatternErrorMessage = useCallback(
		(fieldName: string, pattern: RegExp): string => {
	...
	const validateField = useCallback(
		...
		[validationRules, getPatternErrorMessage],
	);
...
export const authValidationRules = {
	...
	registerRules: {
		...
		confirmPassword: {
			required: true,
			custom: (value: string, formValues?: Record<string, unknown>) => {
				if (
					formValues &&
					value !== (formValues as Record<string, string>).password
				) {

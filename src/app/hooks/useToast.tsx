...
export function useToast() {
  ...
  const created = useCallback((item: string, message?: string) => {
    success(`${item} Oluşturuldu`, message || `${item} başarıyla oluşturuldu.`);
  }, [success]);

  const updated = useCallback((item: string, message?: string) => {
    success(`${item} Güncellendi`, message || `${item} başarıyla güncellendi.`);
  }, [success]);

  const deleted = useCallback((item: string, message?: string) => {
    success(`${item} Silindi`, message || `${item} başarıyla silindi.`);
  }, [success]);

  const saved = useCallback((item: string, message?: string) => {
